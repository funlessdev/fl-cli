...
func TestModCreate(t *testing.T) {
	...
	t.Run("should use ModService.Create to create modules", func(t *testing.T) {
		...
		mockModHandler.On("Create", mock.Anything, testMod).Return(nil)

		err := cmd.Run(testCtx, mockModHandler, testLogger, &Mod{})
		require.NoError(t, err)
		mockModHandler.AssertCalled(t, "Create", mock.Anything, testMod)
	...
	t.Run("should return error if invalid create request", func(t *testing.T) {
		...
		mockModHandler.On("Create", mock.Anything, testMod).Return(e)
